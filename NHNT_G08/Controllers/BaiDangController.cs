...
namespace NHNT_G08.Controllers
{
    public class BaiDangController : Controller
    {
        ...
        // Tìm kiếm bài đăng 
        public JsonResult SearchProduct(string searchString)
        {
            var timKiem = _context.tblPhong.ToList();
            if (String.IsNullOrEmpty(searchString))
            {
                return Json(timKiem);
            }
            timKiem = timKiem.Where(p => p.diaChi.Contains(searchString) && p.maTaiKhoan == Convert.ToInt32(_httpContextAccessor.HttpContext.Session.GetString("maTaiKhoan"))).ToList();
            LayThongTinNguoiDang(timKiem);
            //timKiem = timKiem.Where(pro => pro.diaChi.Contains(searchString));
            return Json(timKiem);
        }
    }
}
